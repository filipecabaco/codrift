defmodule Codrift.ConductorTest do
  ...
  alias Codrift.{AgentSupervisor, Conductor}
  ...
  describe "fan-out mode" do
    ...
    test "subscribe delivers :conductor_agent_ready when agent goes idle", %{
      agent_sup: sup,
      dir: dir
    } do
    ...
    test "send_to routes only to the named agent", %{agent_sup: sup, dir: dir} do
      ...
      refute_receive {:conductor_output, "init-sendto", _, _}, 200
    end
    ...
    test "crashing sub-agent does not crash the conductor", %{agent_sup: sup, dir: dir} do
      ...
      [agent_pid] = AgentSupervisor.list_agents(sup)
  ...
  describe "orchestrator mode" do
    test "starts exactly one agent (the orchestrator) in the context dir", %{
      agent_sup: sup,
      dir: ctx
    } do
      pid =
        start_conductor("init-orch", [ctx],
          agent_sup: sup,
          task: "do the thing",
          context_dir: ctx
        )
    ...
    test "planning prompt includes orchestration.md content when present", %{
      agent_sup: sup,
      dir: ctx
    } do

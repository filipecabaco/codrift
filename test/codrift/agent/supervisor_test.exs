defmodule Codrift.AgentSupervisorTest do
  ...
  test "start_agent returns {:ok, pid}", %{sup: sup} do
    assert {:ok, pid} = AgentSupervisor.start_agent("init-1", System.tmp_dir!(), EchoAdapter, sup)
    assert is_pid(pid)
    assert Process.alive?(pid)
  end

  test "list_agents returns running agent pids", %{sup: sup} do
    {:ok, pid1} = AgentSupervisor.start_agent("init-1", System.tmp_dir!(), EchoAdapter, sup)
    {:ok, pid2} = AgentSupervisor.start_agent("init-1", System.tmp_dir!(), EchoAdapter, sup)
  ...
  test "stop_agent removes agent from list", %{sup: sup} do
    {:ok, pid} = AgentSupervisor.start_agent("init-1", System.tmp_dir!(), EchoAdapter, sup)
  ...
  test "agents for different initiatives can coexist", %{sup: sup} do
    {:ok, pid1} = AgentSupervisor.start_agent("init-a", System.tmp_dir!(), EchoAdapter, sup)
    {:ok, pid2} = AgentSupervisor.start_agent("init-b", System.tmp_dir!(), EchoAdapter, sup)

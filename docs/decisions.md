...
| CLI in release | `eval` dispatch, not `start` | Each `codrift <subcommand>` forks a short-lived process via `bin/codrift eval 'Module.run(System.argv())'`; the full supervision tree is NOT started — only the code is loaded; avoids port conflicts when TUI is already running; TUI command is the sole exception (calls `Application.ensure_all_started/1` explicitly) |

...
| 49 | `codrift <file…>` quick-open | **Temporary initiative from CLI args.** `codrift file1.ex lib/foo/bar.ex` (one or more paths) launches the TUI with an ephemeral initiative (name `tmp-<timestamp>`, status `:planning`, not persisted to disk on open). The TUI opens directly in Context mode showing the resolved file paths as the initiative's dirs. A status-bar hint (`P: promote`) reminds the user the initiative is temporary. Pressing `P` opens a small inline prompt to enter a real name; on confirm the initiative is saved with that name and the proper folder paths derived from the file arguments (each file's parent directory, deduped). Files that share a common ancestor dir are grouped under that dir; single-file invocations use its parent as the sole dir. The `Codrift.CLI.TUI` entrypoint detects positional file args, calls `Initiative.create_temp/1`, then starts the TUI with `temp?: true` in state. |
